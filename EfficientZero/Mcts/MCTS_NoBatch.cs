...
public class MCTS_NoBatch
{
    ...
    public MCTS_NoBatch(Environ<Space, Space> env, IEfficientZeroNet model, Config config, DeviceType deviceType = DeviceType.CUDA)
    {
        Env = env;
        ...
        */
        ...
        for (int batchIndex = 0; batchIndex < numberOfBatches; batchIndex++)
        {
            ...
            foreach (var batchData in batch)
            {
                ...
                var initValue = ScalarConverter.ScalarTransform(initValueScalar.squeeze(0), halfWidth: _config.support_width).cuda();
                ...
                Tensor initPolicyLoss = -(F.log_softmax(initPolicy, 1) * initTargetPolicy).sum();
                Tensor initValueLoss = -(F.log_softmax(initValueScalar, 1) * initPredTargetValues).sum();
                ...
                for (int i = 1; i < rolloutDepth; i++)
                {
                    ...
                    Tensor policyLoss = -(F.log_softmax(policy, 1) * targetPolicy).sum();
                    Tensor valueLoss = -(F.log_softmax(value, 1) * predTargetValues).sum();
                    Tensor valuePrefixLoss = -(F.log_softmax(valuePrefix, 1) * predTargetValuePrefix).sum();
        ...
        var metricsDict = new Dictionary<string, float>
        {
            ...
            { "Loss/value", totalValueLoss.ToSingle() * _valueWeight },
