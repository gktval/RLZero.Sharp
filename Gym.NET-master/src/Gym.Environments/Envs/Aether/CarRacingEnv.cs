...
namespace Gym.Environments.Envs.Aether
{
    ...
    public class CarRacingEnv : Env
    {
        ...
        public override Image Render(string mode = "human")
        {
            ...
            {
                var resizeImg = img.Clone();
                resizeImg.Mutate(x => x.Resize(STATE_W, STATE_H));
                // Convert the image to an NDArray
                _LastImageArray = new float[STATE_W, STATE_H, 3];
                for (int j = 0; j < STATE_H; j++)
                {
                    for (int i = 0; i < STATE_W; i++)
                    {
                        _LastImageArray[i, j, 0] = (float)resizeImg[i, j].R / 255f;
                        _LastImageArray[i, j, 1] = (float)resizeImg[i, j].G / 255f;
                        _LastImageArray[i, j, 2] = (float)resizeImg[i, j].B / 255f;
                    }
                }
            }
        ...
        public override Step Step(object action)
        {
            ...
            if (!null_action) // First step without action, called from reset()
            {
                ...
                Car.FuelSpent = 0f;
                step_reward = Reward - PreviousReward;
